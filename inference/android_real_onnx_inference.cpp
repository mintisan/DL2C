...
class AndroidRealONNXInference {
private:
...
public:
    AndroidRealONNXInference() : ort_api(nullptr), env(nullptr), session(nullptr), 
                                session_options(nullptr), memory_info(nullptr) {
        std::cout << "=== Android 真实 ONNX Runtime C++ 推理测试 ===" << std::endl;
        std::cout << "使用真正的 ONNX Runtime Android 版本" << std::endl;
    }

    ~AndroidRealONNXInference() {
        cleanup();
    }

    bool initialize(const std::string& model_file = "models/mnist_model.onnx") {
        model_path = model_file;
        
        std::cout << "初始化 ONNX Runtime..." << std::endl;
        ...
        // 创建环境
        OrtStatus* status = ort_api->CreateEnv(ORT_LOGGING_LEVEL_WARNING, "AndroidInference", &env);
        if (status != nullptr) {
        ...
        if (status != nullptr) {
            std::cerr << "错误: 加载模型失败: " << ort_api->GetErrorMessage(status) << std::endl;
            std::cerr << "模型路径: " << model_path << std::endl;
            ort_api->ReleaseStatus(status);
            return false;
        }
    ...
    std::vector<float> loadTestData(const std::string& filename, int& expected_label) {
        ...
        // ❌ 错误的读取方式 - 文件不包含标签！
        // file.read(reinterpret_cast<char*>(&expected_label), sizeof(int));
        ...
        // 注意：expected_label需要从其他地方获取，这里先设为0
        expected_label = 0; // 临时设置，需要从metadata获取
        return data;
    }
    ...
    void runTests() {
        ...
        std::cout << "\n=== 开始 Android 真实 ONNX Runtime 推理测试 ===" << std::endl;
        ...
        if (!results.empty()) {
            ...
            std::cout << "\n=== Android 真实 ONNX Runtime 推理结果统计 ===" << std::endl;
            ...
            std::cout << "推理速度: " << std::fixed << std::setprecision(1) << 1000.0 / avg_time << " FPS" << std::endl;
                    ...
                    if (label_map.find(idx) != label_map.end()) {
                        ...
                        if (predicted_class != expected_label) {
                            ...
                            std::cout << "  样本 " << std::setw(3) << idx 
                                      << ": 真实=" << expected_label
                                      << ", 预测=" << predicted_class
                                      << ", 时间=" << std::setprecision(3) << inference_time << "ms" << std::endl;
                            shown++;
                        }
                    }
                }
            ...
            // 保存结果到文件
            saveResults(results, accuracy, avg_time, fps);
        } else {
...
private:
    ...
    std::map<int, int> loadLabelsFromMetadata() {
        ...
        if (!metadata_file.is_open()) {
            std::cerr << "❌ 无法打开元数据文件: test_data_mnist/metadata.json" << std::endl;
    ...
    void saveResults(const std::vector<std::pair<int, double>>& results, double accuracy, 
                    double avg_time, double fps) {
        std::ofstream file("results/android_real_onnx_results.txt");
        if (file.is_open()) {
            file << "Android 真实 ONNX Runtime C++ 推理结果\n";
            file << "==========================================\n";
            file << "测试样本数: " << results.size() << "\n";
            file << "准确率: " << std::fixed << std::setprecision(2) << accuracy << "%\n";
            file << "平均推理时间: " << std::fixed << std::setprecision(3) << avg_time << " ms\n";
            file << "推理 FPS: " << std::fixed << std::setprecision(0) << fps << "\n";
            file << "使用框架: ONNX Runtime " << ORT_API_VERSION << " (Android)\n";
            file.close();
            std::cout << "✓ 结果已保存到 results/android_real_onnx_results.txt" << std::endl;
        }
    }
};

int main() {
    std::cout << "启动 Android 真实 ONNX Runtime MNIST 推理程序..." << std::endl;
    
    AndroidRealONNXInference inference;
    ...
    std::cout << "\nAndroid 真实 ONNX Runtime 推理测试完成！" << std::endl;
    return 0;
} 